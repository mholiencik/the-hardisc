...
`endif
    ...
    assign s_reri_fetch.valid    = s_reri_fetch.ce || ((s_imiscon_i[0] == IMISCON_FERR) & s_ibus_rst_en[0]) || s_reri_fetch.uec;  // new error present
    assign s_reri_fetch.ce       = (s_imiscon_i[0] == IMISCON_FCCE);     // corrected error
    assign s_reri_fetch.ued      = 1'b0;    // uncorrected deferred
    assign s_reri_fetch.uec      = (s_imiscon_i[0] == IMISCON_FUCE);    // uncorrected critical
    assign s_reri_fetch.ec       = ((s_imiscon_i[0] == IMISCON_FERR) & s_ibus_rst_en[0]) ? 8'd15 : 8'd14;     // error code (Table 6)
    ...
    assign s_reri_pipe.valid    = s_prior_rstpp[i] || s_ex_discrepancy[i] || s_bru_discrepancy[i];  // new error present
